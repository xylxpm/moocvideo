    ...
    Suggest.prototype = {
        ...
        getLatestSuggest: function() { // 最近的0-5条搜索记录
            ...
            $.ajax({
                ...
                dataType: "json",
                success: function(res) {
                    res.data.length && that.autoComplete(res.data);
                }
            });
        },
        ...
        search: function(keyword) { // 搜索主体方法
            switch( OP_CONFIG.module ){
                case "wenda" :
                    location.href = "/search/wenda?words=" + encodeURIComponent(keyword);
                    break;

                case "article" :
                    location.href = "/search/article?words=" + encodeURIComponent(keyword);
                    break;

                case "course" :
                    location.href = "/search/course?words=" + encodeURIComponent(keyword);
                    break;

                default:
                    location.href = "/search/?words=" + encodeURIComponent(keyword);
            }
        },
