        ...
        var T = function () {
            if (OP_CONFIG.userInfo) {
                ...
                return c.mid = pageInfo.mid, w = window.setInterval(a = function () {
                    var a, y;
                    "object" == typeof thePlayer && thePlayer.getCurrentTime && (a = (new Date).getTime(), y = parseInt(a - v) / 1e3, c.time = y - h, c.learn_time = thePlayer.getCurrentTime(),
                        $.ajax({
        ...
        $(document).on("click", "#js-discuss-submit", function () {
            ...
            $(document).on("click", "#js-note-submit", function () {
                ...
                if (!a.hasClass("submit-loading")) {
                    ...
                    $.extend(c, postData), v.remote.note.prev(c), a.addClass("submit-loading").val("正在保存..."),
                        $.ajax({
                            //  url: "/course/addnote",
                            type: "post",
                            dataType: "json",
